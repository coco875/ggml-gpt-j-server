...
void generateResult(gpt_vocab &vocab, gpt_params &params, gptj_model &model, std::string &result, std::mt19937 &rng) {
    int n_past = 0;
...
int main(int argc, char ** argv) {
    ...
    while (1)
    {
        ...
        // read the 4 bytes of the length of the prompt
        params.n_predict = *(int*)buffer;
        printf("n_predict: %d\n", params.n_predict);
        // read the prompt
        for (int i = 0; i < valread - 4; i++) {
            params.prompt += buffer[i + 4];
        }
        std::cout << params.prompt << std::endl;

        generateResult(vocab, params, model, result, rng);
